...
function setCookie(name, value, options = {}) {
    ...
    // Для production можно добавить Secure
...
function deleteCookie(name, options = {}) {
    document.cookie = `${name}=; Path=${options.path || '/'}; Expires=Thu, 01 Jan 1970 00:00:01 GMT;` + (options.domain ? ` Domain=${options.domain};` : '') + (("TURBOPACK compile-time falsy", 0) ? ("TURBOPACK unreachable", undefined) : '');
}
function clearAuthCookies() {
    deleteCookie('access');
    deleteCookie('refresh');
// Добавьте другие куки аутентификации, если они есть
}
}}),
...
apiPrivate.interceptors.request.use((config)=>{
    const accessToken = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$shared$2f$utils$2f$cookie$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["getCookie"])("access");
...
apiPrivate.interceptors.response.use((response)=>response, async (error)=>{
    ...
    if ([
        401,
        403
    ].includes(error.response.status) && error.config && !error.config._isRetry) {
        try {
            console.log(error.response.status);
            const resp = await apiPrivate.post("auth/refresh", {
                refresh: (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$shared$2f$utils$2f$cookie$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["getCookie"])("refresh")
            });
            const newAccessToken = resp.data.access;
            originalRequest.headers.Authorization = `Bearer ${newAccessToken}`;
            // Используем setCookie вместо прямого присваивания
        ...
        // Можно добавить редирект на страницу логина или очистку кук
        }
    }
    throw error;
});
...
//# sourceMappingURL=%5Broot-of-the-server%5D__94fe1fbd._.js.map